...
export type FakeTargetScript = {
  ...
  processes?: Record<string, { pid: number; state: string }>;
  logs?: Record<string, string>;
};

export class FakeCapabilityAdapter implements CapabilityAdapter {
  readonly capability: CapabilityName;

  private readonly script: FakeTargetScript;
  ...
  private async executeFlash(context: CapabilityExecutionContext): Promise<CapabilityExecutionResult> {
  ...
  private async executeWatchSerial(context: CapabilityExecutionContext): Promise<CapabilityExecutionResult> {
    ...
    return result(
      "watch_serial",
      true,
      "fake serial watch completed",
      { log_ref: ref, events: [], patterns_matched: patternsMatched },
      [ref]
    );
  }
  ...
  private async executeWaitAdb(context: CapabilityExecutionContext): Promise<CapabilityExecutionResult> {
    ...
    return result(
      "wait_adb",
      online,
      online ? "fake adb online" : "fake adb offline",
      {
  ...
  private async executeCheckProcess(context: CapabilityExecutionContext): Promise<CapabilityExecutionResult> {
    ...
    return result(
      "check_process",
      true,
      processInfo === undefined ? "fake process not found" : "fake process found",
      {
  ...
  private async executeSaveSnapshot(context: CapabilityExecutionContext): Promise<CapabilityExecutionResult> {
...
export class FakeAdapterRegistry implements CapabilityAdapterRegistry {
  private readonly adapters: Map<CapabilityName, CapabilityAdapter>;

  constructor(script: FakeTargetScript = {}) {
    this.adapters = new Map(
      P0_CAPABILITIES.map(capability => [capability, new FakeCapabilityAdapter(capability, script)] satisfies [CapabilityName, CapabilityAdapter])
    );
  }
