...
export class SerialConnection implements Connection {
  ...
  async connect(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      this.port!.open(err => err ? reject(err) : (this._state = "connected", resolve()));
    });
    this.port.on("close", () => { this._state = "disconnected"; this.onDisconnect?.(); });
    this.port.on("error", () => { this._state = "error"; this.onDisconnect?.(); });
  }

  async disconnect(): Promise<void> {
    if (!this.port?.isOpen) return;
    await new Promise<void>(resolve => this.port!.close(() => { this._state = "disconnected"; resolve(); }));
  }
  ...
  async *stream(timeout: number): AsyncIterable<string> {
    ...
    for await (const line of parser) {
      if (this._state === "disconnected" || Date.now() > deadline) break;
      yield line;
    }
  }
}
